...
const { getNetworkDisplayName } = require('./util')
...
const RPC_PROVIDER_TYPES = [CLASSIC, CALLISTO, LEEP, LEEPTEST]
...
module.exports = class NetworkController extends EventEmitter {
  ...
  async setProviderType (type) {
    ...
    assert(INFURA_PROVIDER_TYPES.includes(type) || RPC_PROVIDER_TYPES.includes(type) || type === LOCALHOST, `NetworkController - Unknown rpc type "${type}"`)
  ...
  _configureProvider (opts) {
    ...
    const isRPC = RPC_PROVIDER_TYPES.includes(type)
    if (isInfura) {
      this._configureInfuraProvider(opts)
    // other type-based rpc endpoints
    } else if (isRPC) {
      const { rpcUrl, chainId, ticker } = networks.networkList[type]
      this._configureStandardProvider({ rpcUrl, chainId, ticker, nickname: getNetworkDisplayName(type) })
